...
def _csv_has_valid_data(csv_path):
    ...
    A file that only has headers / blank lines is NOT considered 'done'."""
...
def run_cast3m(X, run_number):
    ...
    csv_temp = os.path.join(csv_dir, f"temp_results_no_{run_number}_epsilon_{epsilon}_h-hot_{hhot}_lamda_{lamda}_cp_{cp}.csv")
    csv_pg = os.path.join(csv_dir, f"pg_results_no_{run_number}_epsilon_{epsilon}_h-hot_{hhot}_lamda_{lamda}_cp_{cp}.csv")

    # ====================================================================
    # RESUME LOGIC
    # If BOTH result CSVs already exist and contain valid data, skip the
    # (expensive) Cast3M run. Otherwise remove any partial files and run.
    # NOTE: relies on the value lists (and therefore run_number <-> params
    # mapping) being unchanged between runs.
    # ====================================================================
    already_done = _csv_has_valid_data(csv_temp) and _csv_has_valid_data(csv_pg)

    if already_done:
        print(f"\n[SKIP loop no {run_number}] Already computed -> reusing existing CSV.")
    else:
