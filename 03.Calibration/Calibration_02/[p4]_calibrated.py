...
# ===================================================================
# THE 3 STEPS : milestone parameter set of each step (eps, h, cp, lam)
# The CSV matching each set is plotted vs Dauti vs Exp.
# ===================================================================
STEPS = [
    {"name": "Step 1 - reduced KS",            "eps": 0.68, "h": 11.0, "cp": 948, "lam": 1.38,
     "save": "Temp_compare_step1_KS.png"},
    {"name": "Step 2 - re-calibrated epsilon", "eps": 0.6,  "h": 11.0, "cp": 948, "lam": 1.38,
     "save": "Temp_compare_step2_eps.png"},
    {"name": "Step 3 - increased CS (final)",  "eps": 0.6,  "h": 11.0, "cp": 960, "lam": 1.38,
     "save": "Temp_compare_step3_CS.png"},
]
...
def find_csv(eps, h, cp, lam):
    """Return the temperature CSV whose 4 parameters match the target set."""
    for f in glob.glob(os.path.join(csv_dir, "temp_results_no_*.csv")):
        name = os.path.basename(f)
        if (abs((get_val(name, TOK_EPS) or -9) - eps) < 1e-3 and
            abs((get_val(name, TOK_H)   or -9) - h)   < 1e-3 and
            abs((get_val(name, TOK_CP)  or -9) - cp)  < 1e-3 and
            abs((get_val(name, TOK_LAM) or -9) - lam) < 1e-3):
            return f
    return None


# ===================================================================
# PLOT : one step's simulation vs Dauti vs Exp
# ===================================================================
def plot_compare(step):
    csv_file = find_csv(step["eps"], step["h"], step["cp"], step["lam"])
    if csv_file is None:
        print(f"-> No CSV matches {step['name']} "
              f"(eps={step['eps']}, h={step['h']}, cp={step['cp']}, lam={step['lam']}).")
        return
    ...
    plt.title(f"{step['name']}  |  Sim vs Dauti vs Exp\n"
              f"($\\epsilon$={step['eps']:g}, $h_{{hot}}$={step['h']:g}, "
              f"$C_{{ps}}$={step['cp']:g}, $\\lambda_{{d0}}$={step['lam']:g})",
    ...
    save_path = os.path.join(output_dir, step["save"])
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"-> Image saved: {step['save']}  (from {os.path.basename(csv_file)})")
...
if __name__ == "__main__":
    print("Calibration 02 - compare each step vs Dauti vs Exp\n")
    for step in STEPS:
        plot_compare(step)
