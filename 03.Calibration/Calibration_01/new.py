...
output_dir = os.path.join(working_dir, "Cal")
os.makedirs(output_dir, exist_ok=True)

# ===================================================================
# FINAL CHOSEN (SELECTED) PARAMETERS  <-- input the chosen values here
# ===================================================================
SELECTED_EPS = 0.68
SELECTED_H   = 11.0

SHOW_120MM = False     # overlay the 120 mm experimental curve if available
...
# ===================================================================
time_dauti = np.arange(0, 245, 5)   # 0..240, step 5 (49 pts)
time_exp   = np.arange(5, 245, 5)   # 5..240, step 5 (48 pts)
...
def is_selected(eps, h):
    return (eps is not None and h is not None
            and abs(eps - SELECTED_EPS) < 1e-3 and abs(h - SELECTED_H) < 1e-3)


# ===================================================================
# AUTOMATIC PLOTTING (sim vs Dauti vs Exp), one figure per CSV
# ===================================================================
def plot_and_save(file_list, plot_type):
    for file_path in file_list:
        filename = os.path.basename(file_path)
        eps, h = parse_params(filename)
        ...
        # ---- reference overlay (temperature only) ----
        if plot_type == "Temp":
            for d in ACTIVE_DEPTHS:
                if d in DAUTI:
                    plt.plot(time_dauti, DAUTI[d], color=colors[d],
                             linestyle='--', linewidth=1.1)
                if d in EXP:
                    plt.plot(time_exp, EXP[d], color=colors[d],
                             linestyle=':', linewidth=1.6)
            if SHOW_120MM and '120' in EXP:
                plt.plot(time_exp, EXP['120'], color=colors['120'],
                         linestyle=':', linewidth=1.6)

        # ---- title / filename (flag the selected combination) ----
        sel = is_selected(eps, h)
        tag = "  [SELECTED]" if sel else ""
        if eps is not None:
            title = f"{plot_type} | epsilon={eps:g} | h_hot={h:g}{tag}"
            save_name = (f"{plot_type}_epsilon_{eps:g}_h-hot_{h:g}"
                         f"{'_SELECTED' if sel else ''}.png")
        else:
            title = f"{plot_type} - {filename}{tag}"
            save_name = f"{plot_type}_{filename}.png"
        ...
        # ---- two compact legends: depth (colour) + source (linestyle) ----
...
if __name__ == "__main__":
    ...
    print(f"Selected parameters: epsilon={SELECTED_EPS:g}, h_hot={SELECTED_H:g}\n")

    print("--- TEMPERATURE (sim vs Dauti vs Exp) ---")
