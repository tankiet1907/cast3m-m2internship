...
# ===================================================================
# SELECTED PARAMETERS  -> the CSV matching these values is plotted
# ===================================================================
SELECTED_EPS = 0.68
SELECTED_H   = 11.0
...
def find_selected_csv():
    """Return the temperature CSV whose (epsilon, h_hot) match the selected values."""
    for f in glob.glob(os.path.join(csv_dir, "temp_results_no_*.csv")):
        eps, h = parse_params(os.path.basename(f))
        if eps is not None and abs(eps - SELECTED_EPS) < 1e-3 and abs(h - SELECTED_H) < 1e-3:
            return f
    return None
...
def plot_comparison(csv_file):
    ...
    plt.title(f"Temperature | Sim ($\\epsilon$={SELECTED_EPS:g}, "
              f"$h_{{hot}}$={SELECTED_H:g}) vs Dauti vs Exp",
    ...
    save_name = f"Temp_compare_epsilon_{SELECTED_EPS:g}_h-hot_{SELECTED_H:g}.png"
...
if __name__ == "__main__":
    ...
    if csv_file is None:
        print("-> No CSV file matches the selected parameters. "
              "Check SELECTED_EPS / SELECTED_H or the filename tokens.")
    else:
