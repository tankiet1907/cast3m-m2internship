...
def plot_comparison(csv_file):
    ...
    for dkey, col in depth_columns(df):
        a_val = 1.0 if dkey == '10' else 0.3  # Làm mờ các đường không phải 10mm
        plt.plot(df[x_col], df[col], color=DEPTH_COLORS[dkey],
                 linestyle='-', linewidth=1.3, alpha=a_val)

    # ---- Dauti (dashed) + Exp (dotted) ----
    for d in ACTIVE_DEPTHS:
        a_val = 1.0 if d == '10' else 0.3     # Làm mờ các đường không phải 10mm
        if d in DAUTI:
            plt.plot(time_dauti, DAUTI[d], color=DEPTH_COLORS[d], 
                     linestyle='--', linewidth=1.1, alpha=a_val)
        if d in EXP:
            plt.plot(time_exp, EXP[d], color=DEPTH_COLORS[d], 
                     linestyle=':', linewidth=1.6, alpha=a_val)
    ...
    # ---- two compact legends ----
    # Cập nhật alpha cho chú thích để đồng bộ với biểu đồ
    depth_handles = [Line2D([0], [0], color=DEPTH_COLORS[d], lw=1.3, 
                            label=f"{d}mm", alpha=(1.0 if d == '10' else 0.3))
                     for d in ACTIVE_DEPTHS]
