<!--
    This file is automatically generated by installation process.
    You can edit its content, or better override it by providing
    your own file :
    * either copy this file in $HOME/openturns/etc
    and edit this one with your defaults
    * or put a modified copy of this file in your favorite path and
    set the environment variable OPENTURNS_CONFIG_PATH with its
    path
  -->

<!DOCTYPE configuration>
  <openturns-configuration version="1" >

  <!-- TBB-ThreadsNumber is set by default to the number of cores of the machine.
       To set manually the number of threads used for parallel computing, uncomment the following line:
    -->
  <!--<TBB-ThreadsNumber    value_int="1"    />-->
  <Cache-MaxSize          value_int="65536" />

  <!-- OT::Os parameters -->
  <Os-RemoveFiles    value_bool="true"  />

  <!-- OT::XMLStorageManager parameters -->
  <XMLStorageManager-DefaultCompressionLevel value_int="0" />

  <!-- OT::Collection parameters -->
  <Collection-size-visible-in-str-from value_int="10" />
    
  <!-- OT::SpecFunc parameters -->
  <SpecFunc-Precision        value_float="2.0e-16"        />
  <SpecFunc-MaximumIteration value_int="1000" />

  <!-- OT::SymbolicParser parameters -->
  <SymbolicParser-Backend     value_str="ExprTk" />

  <!-- OT::SymbolicParserExprTk parameters -->
  <SymbolicParserExprTk-SmallSize value_int="100" />
  <SymbolicParserExprTk-MaxStackDepth value_int="400" />
  <SymbolicParserExprTk-MaxNodeDepth value_int="10000" />
  <SymbolicParserExprTk-DisableCommutativeCheck value_bool="true" />

  <!-- OT::SymbolicParserMuParser parameters -->
  <SymbolicParserMuParser-SmallSize value_int="1000" />

  <!-- OT::DesignProxy parameters -->
  <DesignProxy-DefaultCacheSize value_int="16777216" />

  <!-- OT::KDTree parameters -->
  <KDTree-leaf_max_size value_int="10" />
  <!--KDTree-n_thread_build value_int="1" /-->

  <!-- OT::KFold parameters -->
  <KFold-DefaultK value_int="10" />

  <!-- OT::KFoldSplitter parameters -->
  <KFoldSplitter-Randomize value_bool="false" />

  <!-- OT::BlendedStep parameters -->
  <BlendedStep-DefaultEta value_float="1.0" />

  <!-- OT::CenteredFiniteDifferenceGradient parameters -->
  <CenteredFiniteDifferenceGradient-DefaultEpsilon value_float="1.0e-5" />

  <!-- OT::CenteredFiniteDifferenceHessian parameters -->
  <CenteredFiniteDifferenceHessian-DefaultEpsilon value_float="1.0e-4" />

  <!-- OT::NonCenteredFiniteDifferenceGradient parameters -->
  <NonCenteredFiniteDifferenceGradient-DefaultEpsilon value_float="1.0e-7" />

  <!-- OT::PiecewiseHermiteEvaluation parameters -->
  <PiecewiseHermiteEvaluation-EpsilonRegular value_float="1.0e-12" />
  <PiecewiseHermiteEvaluation-DefaultEnableExtrapolation value_bool="true"/>
   
  <!-- OT::PiecewiseHermiteEvaluation parameters -->
  <PiecewiseLinearEvaluation-EpsilonRegular value_float="1.0e-12" />
  <PiecewiseLinearEvaluation-DefaultEnableExtrapolation value_bool="true"/>
    
  <!-- OT::UniVariatePolynomialImplementation parameters -->
  <UniVariatePolynomial-SmallDegree value_int="400" />

  <!-- OT::Pie parameters -->
  <Pie-HorizontalMargin value_float="0.3"  />
  <Pie-VerticalMargin   value_float="0.1"  />

  <!-- OT::DrawableImplementation parameters -->
  <Drawable-DefaultLineWidth    value_float="1.0"          />
  <Drawable-AlternativePattern  value_str="S"              />
  <Drawable-DefaultFillStyle    value_str="solid"          />
  <Drawable-DefaultLineStyle    value_str="solid"          />
  <Drawable-DefaultPaletteName  value_str="Tableau"        />
  <Drawable-DefaultPattern      value_str="s"              />
  <Drawable-DefaultPointStyle   value_str="plus"           />
  <Drawable-DefaultPalettePhase value_int="12"             />

  <!-- OT::Text parameters -->
  <Text-DefaultTextSize    value_float="0.75"            />

  <!-- OT::GraphImplementation parameters -->
  <Graph-DefaultHorizontalMargin value_float="0.05"          />
  <Graph-DefaultLegendFontSize   value_float="10.0"          />
  <Graph-DefaultVerticalMargin   value_float="0.05"          />

  <!-- OT::Contour parameters -->
  <Contour-DefaultLevelsNumber value_int="10" />
  <Contour-DefaultIsFilled value_bool="false"        />
  <Contour-DefaultDrawLabels value_bool="true"       />
  <Contour-DefaultColorMapNorm value_str="linear"            />
  <Contour-DefaultColorMap value_str="viridis"       />
  <Contour-DefaultColorBarPosition value_str="right" />
  <Contour-DefaultExtend value_str="both"            />
  <Contour-DefaultAlpha value_float="1.0"            />

  <!-- OT::Cross cuts parameters -->
  <CrossCuts-DefaultWithMonoDimensionalCuts value_bool="true" />

  <!-- OT::IntervalMesher parameters -->
  <IntervalMesher-UseDiamond value_bool="false" />

  <!-- OT::LevelSetMesher parameters -->
  <LevelSetMesher-SolveEquation value_bool="true" />

  <!-- OT::PointToPointEvaluation parameters -->
  <PointToPointEvaluation-BlockSize value_int="256" />

  <!-- OT:: FieldToPointConnection parameters -->
  <FieldToPointConnection-BlockSize value_int="256" />

  <!-- OT:: FieldFunctionalChaosAlgorithm parameters -->
  <FieldFunctionalChaosAlgorithm-DefaultRecompress value_bool="false" />
  
  <!-- OT:: FieldToPointFunctionalChaosAlgorithm parameters -->
  <FieldToPointFunctionalChaosAlgorithm-CopulaType value_str="Normal" />

  <!-- OT:: PointToFieldFunctionalChaosAlgorithm parameters -->
  <PointToFieldFunctionalChaosAlgorithm-DefaultRecompress value_bool="false" />
  <PointToFieldFunctionalChaosAlgorithm-Expansion value_str="LeastSquaresExpansion" />

  <!-- OT::SQP parameters -->
  <SQP-DefaultOmega                   value_float="1.0e-4"           />
  <SQP-DefaultSmooth                  value_float="1.2"              />
  <SQP-DefaultTau                     value_float="0.5"              />

  <!-- OT::TNC parameters -->
  <TNC-DefaultAccuracy value_float="1.0e-4"       />
  <TNC-DefaultEta      value_float="0.25"         />
  <TNC-DefaultFmin     value_float="1.0"          />
  <TNC-DefaultRescale  value_float="1.3"          />
  <TNC-DefaultStepmx   value_float="10.0"         />
  <TNC-DefaultMaxCGit  value_int="50" />

  <!-- OT::AbdoRackwitz parameters -->
  <AbdoRackwitz-DefaultOmega                   value_float="1.0e-4" />
  <AbdoRackwitz-DefaultSmooth                  value_float="1.2"    />
  <AbdoRackwitz-DefaultTau                     value_float="0.5"    />

  <!-- OT::MultiStart parameters -->
  <MultiStart-KeepResults                      value_bool="true" />

  <!-- OT::NLopt parameters -->
  <NLopt-InitialSeed value_int="0" />

  <!-- OT::OptimizationAlgorithm parameters -->
  <OptimizationAlgorithm-DefaultMaximumAbsoluteError    value_float="1.0e-5"        />
  <OptimizationAlgorithm-DefaultMaximumConstraintError  value_float="1.0e-5"        />
  <OptimizationAlgorithm-DefaultMaximumRelativeError    value_float="1.0e-5"        />
  <OptimizationAlgorithm-DefaultMaximumResidualError    value_float="1.0e-5"        />
  <OptimizationAlgorithm-DefaultMaximumTimeDuration     value_float="-1.0" />
  <OptimizationAlgorithm-DefaultMaximumCallsNumber      value_int="1000" />
  <OptimizationAlgorithm-DefaultMaximumIterationNumber  value_int="100" />
  <OptimizationAlgorithm-DefaultCheckStatus             value_bool="true" />

  <!-- OT::Pagmo parameters -->
  <Pagmo-InitialSeed value_int="0" />
  <Pagmo-UnconstrainMethod value_str="death penalty" />
  <Pagmo-memory value_bool="false" />
  <Pagmo-DefaultIncrementalEvolution value_bool="false" />
  <!-- gaco -->
  <Pagmo-gaco-ker value_int="63" />
  <Pagmo-gaco-q value_float="1.0" />
  <Pagmo-gaco-oracle value_float="0.0" />
  <Pagmo-gaco-acc value_float="0.01" />
  <Pagmo-gaco-threshold value_int="1" />
  <Pagmo-gaco-n_gen_mark value_int="7" />
  <Pagmo-gaco-impstop value_int="100000" />
  <Pagmo-gaco-focus value_float="0.0" />
  <!-- de -->
  <Pagmo-de-F value_float="0.8" />
  <Pagmo-de-CR value_float="0.9" />
  <Pagmo-de-variant value_int="2" />
  <Pagmo-sade-variant value_int="2" />
  <Pagmo-sade-variant_adptv value_int="1" />
  <Pagmo-de1220-variant_adptv value_int="1" />
  <!-- ihs -->
  <Pagmo-ihs-phmcr value_float="0.85" />
  <Pagmo-ihs-ppar_min value_float="0.35" />
  <Pagmo-ihs-ppar_max value_float="0.99" />
  <Pagmo-ihs-bw_min value_float="1e-5" />
  <Pagmo-ihs-bw_max value_float="1.0" />
  <!-- pso -->
  <Pagmo-pso-omega value_float="0.7298" />
  <Pagmo-pso-eta1 value_float="2.05" />
  <Pagmo-pso-eta2 value_float="2.05" />
  <Pagmo-pso-max_vel value_float="0.5" />
  <Pagmo-pso-variant value_int="5" />
  <Pagmo-pso-neighb_type value_int="2" />
  <Pagmo-pso-neighb_param value_int="4" />
  <!-- sga -->
  <Pagmo-sga-cr value_float="0.9" />
  <Pagmo-sga-eta_c value_float="1.0" />
  <Pagmo-sga-m value_float="0.01" />
  <Pagmo-sga-param_m value_float="1.0" />
  <Pagmo-sga-param_s value_int="2" />
  <Pagmo-sga-crossover value_str="exponential" />
  <Pagmo-sga-mutation value_str="polynomial" />
  <Pagmo-sga-selection value_str="tournament" />
  <!-- simulated_annealing -->
  <Pagmo-simulated_annealing-Ts value_float="10.0" />
  <Pagmo-simulated_annealing-Tf value_float="0.1" />
  <Pagmo-simulated_annealing-n_T_adj value_int="10" />
  <Pagmo-simulated_annealing-n_range_adj value_int="1" />
  <Pagmo-simulated_annealing-bin_size value_int="20" />
  <Pagmo-simulated_annealing-start_range value_float="1.0" />
  <!-- bee_colony -->
  <Pagmo-bee_colony-limit value_int="20" />
  <!-- cmaes -->
  <Pagmo-cmaes-cc value_float="-1.0" />
  <Pagmo-cmaes-cs value_float="-1.0" />
  <Pagmo-cmaes-c1 value_float="-1.0" />
  <Pagmo-cmaes-cmu value_float="-1.0" />
  <Pagmo-cmaes-sigma0 value_float="0.5" />
  <!-- xnes -->
  <Pagmo-xnes-eta_mu value_float="-1.0" />
  <Pagmo-xnes-eta_sigma value_float="-1.0" />
  <Pagmo-xnes-eta_b value_float="-1.0" />
  <Pagmo-xnes-sigma0 value_float="-1.0" />
  <!-- nsga2 -->
  <Pagmo-nsga2-cr value_float="0.95" />
  <Pagmo-nsga2-eta_c value_float="10.0" />
  <Pagmo-nsga2-m value_float="0.01" />
  <Pagmo-nsga2-eta_m value_float="50.0" />
  <!-- moead -->
  <Pagmo-moead-weight_generation value_str="grid" />
  <Pagmo-moead-decomposition value_str="tchebycheff" />
  <Pagmo-moead-neighbours value_int="20" />
  <Pagmo-moead-CR value_float="1.0" />
  <Pagmo-moead-F value_float="0.5" />
  <Pagmo-moead-eta_m value_float="20.0" />
  <Pagmo-moead-realb value_float="0.9" />
  <Pagmo-moead-limit value_int="2" />
  <Pagmo-moead-preserve_diversity value_bool="true" />
  <!-- mhaco -->
  <Pagmo-mhaco-ker value_int="63" />
  <Pagmo-mhaco-q value_float="1.0" />
  <Pagmo-mhaco-threshold value_int="1" />
  <Pagmo-mhaco-n_gen_mark value_int="7" />
  <Pagmo-mhaco-focus value_float="0.0" />
  <!-- nspso -->
  <Pagmo-nspso-omega value_float="0.6" />
  <Pagmo-nspso-c1 value_float="2.0" />
  <Pagmo-nspso-c2 value_float="2.0" />
  <Pagmo-nspso-chi value_float="1.0" />
  <Pagmo-nspso-v_coeff value_float="0.5" />
  <Pagmo-nspso-leader_selection_range value_int="60" />
  <Pagmo-nspso-diversity_mechanism value_str="crowding distance" />

  <!-- OT::EfficientGlobalOptimization parameters -->
  <EfficientGlobalOptimization-DefaultAEITradeoff               value_float="1.0"           />
  <EfficientGlobalOptimization-DefaultCorrelationLengthFactor   value_float="1.0"           />
  <EfficientGlobalOptimization-DefaultMultiStartExperimentSize  value_int="100" />
  <EfficientGlobalOptimization-DefaultMultiStartNumber          value_int="20"  />
  <EfficientGlobalOptimization-DefaultParameterEstimationPeriod value_int="1"   />
  <EfficientGlobalOptimization-DefaultOptimizationAlgorithm     value_str="Cobyla" />

  <!-- OT::LOLAVoronoi parameters -->
  <LOLAVoronoi-HybridScoreTradeoff value_float="0.5" />
  <LOLAVoronoi-UseTruncatedDistribution value_bool="false" />
  <LOLAVoronoi-MaximumCombinationsNumber value_int="100" />
  <LOLAVoronoi-DefaultNeighbourhoodCandidatesNumber value_int="15" />
  <LOLAVoronoi-DefaultVoronoiMinimumSamplingSize value_int="1000" />
  <LOLAVoronoi-DefaultVoronoiMeanSamplingSize value_int="10" />
  <LOLAVoronoi-DecompositionMethod value_str="Cholesky" />
  <LOLAVoronoi-NonLinearityAggregationMethod value_str="Maximum" />

  <!-- OT::Cobyla parameters -->
  <Cobyla-DefaultRhoBeg value_float="0.1" />
  
  <!-- OT::Dlib parameters -->
  <Dlib-DefaultInitialTrustRegionRadius value_float="1.0"     />
  <Dlib-DefaultWolfeRho                 value_float="0.01"    />
  <Dlib-DefaultWolfeSigma               value_float="0.9"     />
  <Dlib-DefaultMaxLineSearchIterations  value_int="100"       />

  <!-- OT::SolverImplementation parameters -->
  <Solver-DefaultAbsoluteError             value_float="1.0e-5"        />
  <Solver-DefaultRelativeError             value_float="1.0e-5"        />
  <Solver-DefaultResidualError             value_float="0.0   "        />
  <Solver-DefaultMaximumFunctionEvaluation value_int="100" />

  <!-- OT::CubaIntegration parameters -->
  <CubaIntegration-DefaultMaximumAbsoluteError value_float="1.0e-4" />
  <CubaIntegration-DefaultMaximumRelativeError value_float="1.0e-4" />
  <CubaIntegration-divonne-border value_float="0.0" />
  <CubaIntegration-divonne-maxchisq value_float="10.0" />
  <CubaIntegration-divonne-mindeviation value_float="0.25" />
  <CubaIntegration-suave-flatness value_float="25.0" />
  <CubaIntegration-DefaultMaximumCallsNumber value_int="1000000" />
  <CubaIntegration-mineval value_int="0" />
  <CubaIntegration-seed value_int="0" />
  <CubaIntegration-flags value_int="0" />
  <CubaIntegration-cuhre-key value_int="0" />
  <CubaIntegration-divonne-key1 value_int="47" />
  <CubaIntegration-divonne-key2 value_int="1" />
  <CubaIntegration-divonne-key3 value_int="1" />
  <CubaIntegration-divonne-maxpass value_int="5" />
  <CubaIntegration-divonne-ngiven value_int="0" />
  <CubaIntegration-divonne-nextra value_int="0" />
  <CubaIntegration-suave-nnew value_int="1000" />
  <CubaIntegration-suave-nmin value_int="2" />
  <CubaIntegration-vegas-nstart value_int="1000" />
  <CubaIntegration-vegas-nincrease value_int="500" />
  <CubaIntegration-vegas-nbatch value_int="1000" />
  <CubaIntegration-vegas-gridno value_int="0" />

  <!-- OT::GaussKronrod parameters -->
  <GaussKronrod-MaximumError        value_float="1.0e-12"       />
  <GaussKronrod-MaximumSubIntervals value_int="100" />

  <!-- OT::GaussLegendre parameters -->
  <GaussLegendre-DefaultMarginalIntegrationPointsNumber value_int="64" />

  <!-- OT::FejerExperiment parameters -->
  <FejerExperiment-DefaultMarginalNodesNumber value_int="64" />

  <!-- OT::IteratedQuadrature parameters -->
  <IteratedQuadrature-MaximumError            value_float="1.0e-7" />
  <IteratedQuadrature-Rule                    value_str="G3K7"     />
  <IteratedQuadrature-MaximumSubIntervals     value_int="32"       />

  <!-- OT::Fehlberg parameters -->
  <Fehlberg-InitialStep    value_float="1.0e-7"                       />
  <Fehlberg-LocalPrecision value_float="1.0e-10"                      />
  <Fehlberg-MinimalStep    value_float="1.0e-7"                       />
  <Fehlberg-DefaultOrder   value_int="4" />

  <!-- OT::KarhunenLoeveQuadratureAlgorithm parameters -->
  <KarhunenLoeveQuadratureAlgorithm-RegularizationFactor value_float="0.0" />

  <!-- OT::KarhunenLoeveSVDAlgorithm parameters -->
  <KarhunenLoeveSVDAlgorithm-UseRandomSVD         value_bool="false"            />
  <KarhunenLoeveSVDAlgorithm-RandomSVDVariant     value_str="Halko2010"      />
  <KarhunenLoeveSVDAlgorithm-RandomSVDMaximumRank value_int="1000" />
  <KarhunenLoeveSVDAlgorithm-Halko2011Margin      value_int="2"    />
  <KarhunenLoeveSVDAlgorithm-Halko2011Iterations  value_int="2"    />

  <!-- OT::KarhunenLoeveP1Algorithm parameters -->
  <KarhunenLoeveP1Algorithm-RegularizationFactor value_float="0.0" />
  <KarhunenLoeveP1Algorithm-CovarianceMatrixStorage value_str="DENSE" />
  <KarhunenLoeveP1Algorithm-EigenvaluesSolver value_str="LAPACK" />

  <!-- OT::AdaptiveStieltjesAlgorithm parameters -->
  <AdaptiveStieltjesAlgorithm-MaximumError                    value_float="1.0e-12"      />
  <AdaptiveStieltjesAlgorithm-MaximumSubIntervalsBetweenRoots value_int="64" />

  <!-- OT::LinearModelTest parameters -->
  <LinearModelTest-DefaultHarrisonMcCabeBreakpoint     value_float="0.5"            />
  <LinearModelTest-DefaultLevel                        value_float="0.05"           />
  <LinearModelTest-DefaultDurbinWatsonHypothesis       value_str="Equal"                 />
  <LinearModelTest-DefaultHarrisonMcCabeSimulationSize value_int="1000" />

  <!-- OT::Last parameters -->
  <Last-DefaultMaximumSize value_int="65536" />

  <!-- OT::Compact parameters -->
  <Compact-DefaultHalfMaximumSize value_int="1024" />

  <!-- OT::FaureSequence parameters -->
  <FaureSequence-InitialSeed value_int="1" />

  <!-- OT::HaltonSequence parameters -->
  <HaltonSequence-Scrambling value_str="NONE" />
  <HaltonSequence-InitialSeed value_int="1" />

  <!-- OT::HaselgroveSequence parameters -->
  <HaselgroveSequence-InitialSeed value_int="1" />

  <!-- OT::LowDiscrepaancySequence parameters -->
  <LowDiscrepancySequence-ScramblingSeed value_int="0" />

  <!-- OT::ReverseHaltonSequence parameters -->
  <ReverseHaltonSequence-InitialSeed value_int="1" />

  <!-- OT::SobolSequence parameters -->
  <SobolSequence-InitialSeed value_int="1" />

  <!-- OT::SobolIndicesExperiment parameters -->
  <SobolIndicesExperiment-SamplingMethod value_str="MonteCarlo" />

  <!-- OT::SobolIndicesAlgorithm parameters -->
  <SobolIndicesAlgorithm-DefaultUseAsymptoticDistribution value_bool="false"          />
  <SobolIndicesAlgorithm-DefaultBootstrapConfidenceLevel  value_float="0.95"          />
  <SobolIndicesAlgorithm-DefaultBlockSize                 value_int="1"   />
  <SobolIndicesAlgorithm-DefaultBootstrapSize             value_int="100" />

  <!-- OT::RankSobolSensitivityAlgorithm parameters -->
  <RankSobolSensitivityAlgorithm-DefaultBootstrapSampleRatio value_float="0.8" />

  <!-- OT::FAST parameters -->
  <FAST-DefaultInterferenceFactor value_int="4" />
  <FAST-DefaultResamplingSize     value_int="1" />

  <!-- OT::HSICEstimator parameters -->
  <HSICEstimator-ParallelPValues value_bool="true" />
  <HSICEstimator-PermutationSize value_int="100" />
  <HSICEstimator-InputCovarianceMatrixCacheSizeMb value_int="1000" />

  <!-- OT::RandomGenerator parameters -->
  <RandomGenerator-InitialSeed value_int="0" />
  
  <!-- OT::CovarianceModelImplementation parameters -->
  <CovarianceModel-DefaultNuggetFactor value_float="1e-12"         />
  <CovarianceModel-DefaultTMax         value_float="5.0"           />
  <CovarianceModel-DefaultTMin         value_float="-5.0"          />
  <CovarianceModel-DefaultPointNumber  value_int="129" />

  <!-- OT::DiracCovarianceModel parameters -->
  <DiracCovarianceModel-CheckUnique value_bool="true"         />

  <!-- OT::SpectralModelImplementation parameters -->
  <SpectralModel-DefaultMaximumFrequency value_float="5.0"  />
  <SpectralModel-DefaultMinimumFrequency value_float="-5.0" />
  <SpectralModel-DefaultFrequencyNumber  value_int="129"    />

  <!-- OT::FieldImplementation parameters -->
  <Field-AutomaticScaling value_bool="true"  />
  <Field-ArrowRatio       value_float="0.01" />
  <Field-ArrowScaling     value_float="1.0"  />
  <Field-LevelNumber      value_int="10"     />

  <!-- OT::SampleImplementation parameters -->
  <Sample-CSVPrecision           value_int="16"         />
  <Sample-PrintEllipsisSize      value_int="3"          />
  <Sample-PrintEllipsisThreshold value_int="1000"       />
  <Sample-SmallKendallTau        value_int="23"         />
  <Sample-CommentMarker          value_str="#"          />
  <Sample-CSVFileSeparator       value_str=";"          />
  <Sample-CSVFormat              value_str="scientific" />
  <Sample-PrintFormat            value_str="{:.7g}"     />

  <!-- OT::DomainImplementation parameters -->
  <Domain-SmallVolume     value_float="1.0e-12"    />
  <Domain-DefaultInColor  value_str="prettyOrange" />
  <Domain-DefaultOutColor value_str="prettyBlue"   />

  <!-- OT::Mesh parameters -->
  <Mesh-BackfaceCulling      value_bool="false"    />
  <Mesh-CheckValidity        value_bool="false"    />
  <Mesh-AmbientFactor        value_float="0.1"     />
  <Mesh-DiffuseFactor        value_float="0.7"     />
  <Mesh-Shininess            value_float="100.0"   />
  <Mesh-SpecularFactor       value_float="0.2"     />
  <Mesh-VertexEpsilon        value_float="1.0e-6"  />
  <Mesh-DefaultThetaX        value_float="0.3"     />
  <Mesh-DefaultThetaY        value_float="0.3"     />
  <Mesh-DefaultThetaZ        value_float="0.3"     />
  <Mesh-LargeSize            value_int="5000"      />
  <Mesh-AmbientColor         value_str="white"     />
  <Mesh-EdgeColor            value_str="#FF7F0E88" />
  <Mesh-FaceColor            value_str="#1F77B488" />
  <Mesh-LightColor           value_str="white"     />

  <!-- OT::BoundingVolumeHierarchy parameters -->
  <BoundingVolumeHierarchy-Strategy  value_str="Mean"         />
  <BoundingVolumeHierarchy-BinNumber value_int="50" />

  <!-- OT::EnclosingSimplexAlgorithm parameters -->
  <EnclosingSimplexAlgorithm-BarycentricCoordinatesEpsilon value_float="1.0e-12" />
  <EnclosingSimplexAlgorithm-LargeDimension value_int="5" />

  <!-- OT::Matrix parameters -->
  <Matrix-size-visible-in-str-from value_int="5" />
  <Matrix-DefaultSmallPivot value_float="1.0e-7"  />
  <Matrix-LargestEigenValueRelativeError value_float="1.0e-4" />
  <Matrix-SymmetryThreshold value_float="1.0e-12" />
  <Matrix-LargestEigenValueIterations value_int="50" />
  <Matrix-MaximalScaling  value_float="1.0e-5" />
  <Matrix-StartingScaling value_float="1.0e-13" />

  <!-- OT::Tensor parameters -->
  <Tensor-size-visible-in-str-from value_int="5" />

  <!-- OT::Tensor parameters -->
  <ComplexTensor-size-visible-in-str-from value_int="6" />

  <!-- OT::BernsteinCopulaFactory parameters -->
  <BernsteinCopulaFactory-alpha                    value_float="1.0"             />
  <BernsteinCopulaFactory-BinNumberSelectionMethod value_str="LogLikelihood"   />
  <BernsteinCopulaFactory-kFraction                value_int="2"     />
  <BernsteinCopulaFactory-MaxM                     value_int="1"     />
  <BernsteinCopulaFactory-MinM                     value_int="2"     />
  <BernsteinCopulaFactory-SamplingSize             value_int="10000" />

  <!-- OT::BurrFactory parameters -->
  <BurrFactory-AbsolutePrecision value_float="1e-12"        />
  <BurrFactory-RelativePrecision value_float="1e-12"        />
  <BurrFactory-ResidualPrecision value_float="1e-12"        />
  <BurrFactory-MaximumIteration  value_int="10" />
  
  <!-- OT::CompoundDistribution parameters -->
  <CompoundDistribution-MarginalIntegrationNodesNumber value_int="48"           />
  <CompoundDistribution-MaximumIntegrationNodesNumber  value_int="100000"       />
  <CompoundDistribution-ContinuousDiscretizationMethod value_str="GaussProduct" />

  <!-- OT::PointConditionalDistribution parameters -->
  <PointConditionalDistribution-InitializeSampling value_bool="true"           />
  <PointConditionalDistribution-InitializeTransformation value_bool="true"     />
  <PointConditionalDistribution-UseSimplifiedVersion value_bool="true"         />
  <PointConditionalDistribution-UseGenericConditionalMethods value_bool="true" />
  <PointConditionalDistribution-UseFullParameters value_bool="false"           />
  <PointConditionalDistribution-RatioUniformCandidateNumber value_int="10000"  />
  <PointConditionalDistribution-SmallDimension value_int="6"                   />
  <PointConditionalDistribution-CubaAlgorithm value_str="cuhre"                />
  <PointConditionalDistribution-OptimizationAlgorithm value_str="Cobyla"       />
  <PointConditionalDistribution-RangeAdaptationMethod value_str="NormalCopula" />

  <!-- OT::PosteriorDistribution parameters -->
  <PosteriorDistribution-OptimizationAlgorithm value_str="Cobyla" />
  <PosteriorDistribution-RatioUniformCandidateNumber value_int="10000" />

  <!-- OT::JointDistribution parameters -->
  <JointDistribution-UseGenericCovarianceAlgorithm value_bool="false" />

  <!-- OT::CompositeDistribution parameters -->
  <CompositeDistribution-SolverEpsilon value_float="1.0e-14"       />
  <CompositeDistribution-StepNumber    value_int="256" />

  <!-- OT::Dirichlet parameters -->
  <Dirichlet-DefaultIntegrationSize  value_int="50"     />
  <Dirichlet-DefaultSamplingSize     value_int="500000" />

  <!-- OT::DirichletFactory parameters -->
  <DirichletFactory-ParametersEpsilon value_float="1.0e-12"      />
  <DirichletFactory-MaximumIteration  value_int="10" />

  <!-- OT::DiscreteCompoundDistribution parameters -->
  <DiscreteCompoundDistribution-DefaultEpsilon value_float="1.0e-12" />

  <!-- OT::ExtremeValueCopula parameters -->
  <ExtremeValueCopula-CheckPickandFunction value_bool="true"           />
  <ExtremeValueCopula-CheckGridSize        value_int="11" />

  <!-- OT::FisherSnedecorFactory parameters -->
  <FisherSnedecorFactory-D1LowerBound value_float="1.0e-2" />
  <FisherSnedecorFactory-D2LowerBound value_float="1.0e-2" />

  <!-- OT::FrechetFactory parameters -->
  <FrechetFactory-BoundMargin value_float="10.0" />

  <!-- OT::GeneralizedExtremeValue parameters -->
  <GeneralizedExtremeValue-XiThreshold value_float="1.0e-6" />
  <GeneralizedExtremeValue-MMin value_float="1.05" />
  <GeneralizedExtremeValue-MMax value_float="1.0e3" />

  <!-- OT::GeneralizedExtremeValueFactory parameters -->
  <GeneralizedExtremeValueFactory-MaximumCallsNumber value_int="10000" />
  <GeneralizedExtremeValueFactory-MaximumAbsoluteError    value_float="1.0e-5"        />
  <GeneralizedExtremeValueFactory-MaximumConstraintError  value_float="1.0e-5"        />
  <GeneralizedExtremeValueFactory-MaximumObjectiveError   value_float="1.0e-5"        />
  <GeneralizedExtremeValueFactory-MaximumRelativeError    value_float="1.0e-5"        />
  <GeneralizedExtremeValueFactory-InitializationMethod value_str="Gumbel" />
  <GeneralizedExtremeValueFactory-NormalizationMethod value_str="MinMax" />
  <GeneralizedExtremeValueFactory-DefaultOptimizationAlgorithm value_str="Cobyla" />

  <!-- OT::ProfileLikelihoodResult parameters -->
  <ProfileLikelihoodResult-AbsolutePrecision value_float="1.0e-10" />
  <ProfileLikelihoodResult-RelativePrecision value_float="1.0e-10" />
  <ProfileLikelihoodResult-ConfidenceIntervalMargin value_float="0.05" />
  <ProfileLikelihoodResult-TextMargin value_float="0.03" />
  <ProfileLikelihoodResult-StartingScaling value_float="1.0e-2" />

  <!-- OT::GeneralizedPareto parameters -->
  <GeneralizedPareto-MMin value_float="1.05" />
  <GeneralizedPareto-MMax value_float="1.0e3" />

  <!-- OT::GeneralizedParetoFactory parameters -->
  <GeneralizedParetoFactory-MaximumAbsoluteError    value_float="1.0e-10"        />
  <GeneralizedParetoFactory-MaximumConstraintError  value_float="1.0e-10"        />
  <GeneralizedParetoFactory-MaximumObjectiveError   value_float="1.0e-10"        />
  <GeneralizedParetoFactory-MaximumRelativeError    value_float="1.0e-10"        />
  <GeneralizedParetoFactory-MeanResidualLifeConfidenceLevel   value_float="0.95" />
  <GeneralizedParetoFactory-ThresholdStabilityConfidenceLevel value_float="0.95" />
  <GeneralizedParetoFactory-MaximumCallsNumber value_int="1000" />
  <GeneralizedParetoFactory-MeanResidualLifePointNumber   value_int="100" />
  <GeneralizedParetoFactory-ThresholdStabilityPointNumber value_int="100" />
  <GeneralizedParetoFactory-SmallSize               value_int="20"   />
  <GeneralizedParetoFactory-InitializationMethod value_str="Generic" />
  <GeneralizedParetoFactory-NormalizationMethod value_str="MinMax" />
  <GeneralizedParetoFactory-DefaultOptimizationAlgorithm value_str="Cobyla" />

  <!-- OT::Gibbs parameters -->
  <Gibbs-DefaultUpdatingMethod       value_int="0"     />

  <!-- OT::HistogramFactory parameters -->
  <HistogramFactory-MaximumBinNumber value_int="1024" />

  <!-- OT::InverseNormalFactory parameters -->
  <InverseNormalFactory-Method value_str="MLE" />

  <!-- OT::InverseWishart parameters -->
  <InverseWishart-CDFScaleFactor value_float="0.5" />

  <!-- OT::KernelMixture parameters -->
  <KernelMixture-PDFCDFDiscretization value_int="1000" />
  <KernelMixture-EnableInterpolation  value_bool="false"   />
  
  <!-- OT::KernelSmoothing parameters -->
  <KernelSmoothing-AbsolutePrecision   value_float="0.0"     />
  <KernelSmoothing-CutOffPlugin        value_float="5.0"     />
  <KernelSmoothing-RelativePrecision   value_float="1.0e-5"  />
  <KernelSmoothing-ResidualPrecision   value_float="1.0e-10" />
  <KernelSmoothing-DefaultShiftScale   value_float="1.0e-5"  />
  <KernelSmoothing-BinNumber           value_int="1024"      />
  <KernelSmoothing-MaximumIteration    value_int="50"        />
  <KernelSmoothing-SmallSize           value_int="250"       />
  
  <!-- OT::LogNormal parameters -->
  <LogNormal-CharacteristicFunctionSmallSigmaThreshold value_float="0.2"           />
  <LogNormal-CharacteristicFunctionIntegrationNodes    value_int="256" />

  <!-- OT::LogNormalFactory parameters -->
  <LogNormalFactory-AbsolutePrecision value_float="1e-12"        />
  <LogNormalFactory-RelativePrecision value_float="1e-12"        />
  <LogNormalFactory-ResidualPrecision value_float="1e-12"        />
  <LogNormalFactory-MaximumIteration  value_int="50" />
  <LogNormalFactory-EstimationMethod  value_int="0"  />

  <!-- MarginalDistribution parameters -->
  <MarginalDistribution-UsePDF              value_bool="true"         />
  <MarginalDistribution-MaximumError        value_float="1.0e-7"      />
  <MarginalDistribution-MaximumSubIntervals value_int="128"           />
  <MarginalDistribution-Rule                value_str="G15K31"         />

  <!-- Meixner parameters -->
  <MeixnerDistribution-MaximumAbsoluteError      value_float="1.0e-12"         />
  <MeixnerDistribution-MaximumConstraintError    value_float="1.0e-12"         />
  <MeixnerDistribution-MaximumObjectiveError     value_float="1.0e-12"         />
  <MeixnerDistribution-MaximumRelativeError      value_float="1.0e-12"         />
  <MeixnerDistribution-CDFDiscretization         value_int="10000" />

  <!-- OT::Mixture parameters -->
  <Mixture-SmallWeight          value_float="1.0e-12"        />
  <Mixture-LargeSize            value_int="20"   />
  <Mixture-PDFCDFDiscretization value_int="1000" />
  <Mixture-SmallSize            value_int="50"   />

  <!-- OT::Multinomial parameters -->
  <Multinomial-eta    value_float="1.0e-9" />
  <Multinomial-smallA value_float="10.0"   />

  <!-- OT::PolyaFactory parameters -->
  <PolyaFactory-AbsolutePrecision value_float="1e-12"        />
  <PolyaFactory-RelativePrecision value_float="1e-12"        />
  <PolyaFactory-ResidualPrecision value_float="1e-12"        />
  <PolyaFactory-MaximumIteration  value_int="50" />

  <!-- OT::Normal parameters -->
  <Normal-MinimumCDFEpsilon              value_float="5.0e-2"             />
  <Normal-MarginalIntegrationNodesNumber value_int="16"       />
  <Normal-MaximumNumberOfPoints          value_int="10000000" />
  <Normal-MinimumNumberOfPoints          value_int="100000"   />
  <Normal-SmallDimension                 value_int="6"        />

  <!-- OT::ParetoFactory parameters -->
  <ParetoFactory-AlphaUpperBound value_float="1000.0" />

  <!-- OT::ProductDistribution parameters -->
  <ProductDistribution-LargeCharacteristicFunctionArgument value_float="10.0" />

  <!-- OT::RiceFactory parameters -->
  <RiceFactory-AbsolutePrecision value_float="1e-12"        />
  <RiceFactory-RelativePrecision value_float="1e-12"        />
  <RiceFactory-ResidualPrecision value_float="1e-12"        />
  <RiceFactory-MaximumIteration  value_int="10" />
  
  <!-- OT::TrapezoidalFactory parameters -->
  <TrapezoidalFactory-RhoBeg           value_float="0.1"            />
  <TrapezoidalFactory-RhoEnd           value_float="1.0e-5"         />
  <TrapezoidalFactory-MaximumIteration value_int="2000" />

  <!-- OT::TruncatedDistribution parameters -->
  <TruncatedDistribution-DefaultThresholdRealization value_float="0.5" />

  <!-- OT::TruncatedNormalFactory parameters -->
  <TruncatedNormalFactory-SigmaLowerBound value_float="1.0e-4" />

  <!-- OT::TruncatedOverMesh parameters -->
  <TruncatedOverMesh-UseRejection         value_bool="false" />
  <TruncatedOverMesh-OptimizationAlgorithm value_str="TNC" />
  <TruncatedOverMesh-MarginalIntegrationNodesNumber value_int="64" />
  <TruncatedOverMesh-MaximumIntegrationNodesNumber  value_int="262144" />

  <!-- OT::MaximumLikelihoodFactory parameters -->
  <MaximumLikelihoodFactory-MaximumAbsoluteError         value_float="1.0e-10"        />
  <MaximumLikelihoodFactory-MaximumConstraintError       value_float="1.0e-10"        />
  <MaximumLikelihoodFactory-MaximumObjectiveError        value_float="1.0e-10"        />
  <MaximumLikelihoodFactory-MaximumRelativeError         value_float="1.0e-10"        />
  <MaximumLikelihoodFactory-DefaultOptimizationAlgorithm value_str="TNC"            />
  <MaximumLikelihoodFactory-MaximumCallsNumber           value_int="1000" />

  <!-- OT::LeastSquaresDistributionFactory parameters -->
  <LeastSquaresDistributionFactory-MaximumAbsoluteError         value_float="1.0e-10"        />
  <LeastSquaresDistributionFactory-MaximumConstraintError       value_float="1.0e-10"        />
  <LeastSquaresDistributionFactory-MaximumObjectiveError        value_float="1.0e-10"        />
  <LeastSquaresDistributionFactory-MaximumRelativeError         value_float="1.0e-10"        />
  <LeastSquaresDistributionFactory-MaximumCallsNumber           value_int="1000" />

  <!-- OT::MarginalUniformOrderStatistics parameters -->
  <MarginalUniformOrderStatistics-LargeCaseCDF       value_int="1000"    />
  <MarginalUniformOrderStatistics-MaximumCallsNumber value_int="1000000" />

  <!-- OT::MethodOfMomentsFactory parameters -->
  <MethodOfMomentsFactory-MaximumAbsoluteError    value_float="1.0e-10"        />
  <MethodOfMomentsFactory-MaximumConstraintError  value_float="1.0e-10"        />
  <MethodOfMomentsFactory-MaximumObjectiveError   value_float="1.0e-10"        />
  <MethodOfMomentsFactory-MaximumRelativeError    value_float="1.0e-10"        />
  <MethodOfMomentsFactory-MaximumCallsNumber      value_int="1000" />

  <!-- OT::QuantileMatchingFactory parameters -->
  <QuantileMatchingFactory-MaximumAbsoluteError    value_float="1.0e-10"        />
  <QuantileMatchingFactory-MaximumConstraintError  value_float="1.0e-10"        />
  <QuantileMatchingFactory-MaximumObjectiveError   value_float="1.0e-10"        />
  <QuantileMatchingFactory-MaximumRelativeError    value_float="1.0e-10"        />
  <QuantileMatchingFactory-MaximumCallsNumber      value_int="1000" />

  <!-- OT::Student parameters -->
  <Student-CDFScaleFactor                 value_float="4.0"    />
  <Student-MinimumCDFEpsilon              value_float="5.0e-2" />
  <Student-MarginalIntegrationNodesNumber value_int="16"       />
  <Student-MaximumNumberOfPoints          value_int="10000000" />
  <Student-MinimumNumberOfPoints          value_int="100000"   />
  <Student-SmallDimension                 value_int="6"        />
  
  <!-- OT::StudentFactory parameters -->
  <StudentFactory-NuMax value_float="1.0e3" />

  <!-- OT::MaximumLikelihoodFactory parameters -->
  <StudentCopulaFactory-MaximumAbsoluteError value_float="1.0e-10" />
  <StudentCopulaFactory-MaximumConstraintError value_float="1.0e-10" />
  <StudentCopulaFactory-MaximumObjectiveError value_float="1.0e-10" />
  <StudentCopulaFactory-MaximumRelativeError value_float="1.0e-10" />
  <StudentCopulaFactory-NuMin value_float="2.0" />
  <StudentCopulaFactory-NuMax value_float="1e2" />
  <StudentCopulaFactory-NuStart value_float="5.0" />
  <StudentCopulaFactory-DefaultOptimizationAlgorithm value_str="Cobyla" />
  <StudentCopulaFactory-MaximumCallsNumber value_int="1000" />
  
  <!-- OT::FiniteDiscreteDistribution parameters -->
  <FiniteDiscreteDistribution-SmallSize value_int="10000" />
  
  <!-- OT::UniformOverMesh parameters -->
  <UniformOverMesh-MarginalIntegrationNodesNumber value_int="64" />
  <UniformOverMesh-MaximumIntegrationNodesNumber  value_int="262144" />

  <!-- OT::VonMisesFactory parameters -->
  <VonMisesFactory-AbsolutePrecision value_float="1e-12"        />
  <VonMisesFactory-RelativePrecision value_float="1e-12"        />
  <VonMisesFactory-ResidualPrecision value_float="1e-12"        />
  <VonMisesFactory-MaximumIteration  value_int="10" />
  
  <!-- OT::Wishart parameters -->
  <Wishart-CDFScaleFactor value_float="0.5" />

  <!-- OT::AliMikhailHaqCopulaFactory parameters -->
  <AliMikhailHaqCopulaFactory-ThetaEpsilon value_float="1.0e-14" />

  <!-- OT::FrankCopulaFactory parameters -->
  <FrankCopulaFactory-AbsolutePrecision value_float="1.0e-14"       />
  <FrankCopulaFactory-RelativePrecision value_float="1.0e-14"       />
  <FrankCopulaFactory-ResidualPrecision value_float="1.0e-14"       />
  <FrankCopulaFactory-MaximumIteration  value_int="100" />

  <!-- OT::LinearCombinationDistribution parameters -->
  <LinearCombinationDistribution-SimplifyAtoms                  value_bool="true"              />
  <LinearCombinationDistribution-DefaultAlpha                   value_float="5.0"             />
  <LinearCombinationDistribution-DefaultBeta                    value_float="8.5"             />
  <LinearCombinationDistribution-DefaultCDFEpsilon              value_float="1.0e-10"         />
  <LinearCombinationDistribution-DefaultPDFEpsilon              value_float="1.0e-10"         />
  <LinearCombinationDistribution-DefaultBlockMax                value_int="16"    />
  <LinearCombinationDistribution-DefaultBlockMin                value_int="3"     />
  <LinearCombinationDistribution-DefaultMaxSize                 value_int="65536" />
  <LinearCombinationDistribution-MaximumSupportSize             value_int="2048"  />
  <LinearCombinationDistribution-ProjectionDefaultSize          value_int="25"    />
  <LinearCombinationDistribution-SmallSize                      value_int="100"   />

  <!-- OT::Evaluation parameters -->
  <Evaluation-ParameterEpsilon   value_float="1.0e-7"        />
  <Evaluation-DefaultPointNumber value_int="129" />

  <!-- OT::DualLinearCombinationEvaluation parameters -->
  <DualLinearCombinationEvaluation-SmallCoefficient value_float="0.0" />

  <!-- OT::LinearCombinationEvaluation parameters -->
  <LinearCombinationEvaluation-SmallCoefficient value_float="0.0" />

  <!-- OT::DistFunc parameters -->
  <DistFunc-Precision        value_float="1.0e-14"        />
  <DistFunc-MaximumIteration value_int="5000" />

  <!-- OT::KFactor parameters -->
  <KFactor-Precision                     value_float="1.0e-8"        />
  <KFactor-DefaultIntegrationNodesNumber value_int="256" />
  <KFactor-MaximumIteration              value_int="32"  />

  <!-- OT::RootStrategyImplementation parameters -->
  <RootStrategy-DefaultMaximumDistance value_float="8.0" />
  <RootStrategy-DefaultStepSize        value_float="1.0" />

  <!-- OT::SimulationAlgorithm parameters -->
  <SimulationAlgorithm-DefaultMaximumCoefficientOfVariation value_float="1.0e-1"         />
  <SimulationAlgorithm-DefaultMaximumStandardDeviation      value_float="0.0"            />
  <SimulationAlgorithm-DefaultMaximumTimeDuration           value_float="-1.0" />
  <SimulationAlgorithm-DefaultBlockSize                     value_int="1"    />
  <SimulationAlgorithm-DefaultMaximumOuterSampling          value_int="1000" />

  <!-- OT::ProbabilitySimulationResult parameters -->
  <ProbabilitySimulationResult-CheckPositiveVariance  value_bool="false"  />
  <ProbabilitySimulationResult-DefaultConfidenceLevel value_float="0.95" />

  <!-- OT::ExpectationSimulationAlgorithm parameters -->
  <ExpectationSimulationAlgorithm-DefaultCoefficientOfVariationCriterionType value_str="MAX"  />
  <ExpectationSimulationAlgorithm-DefaultStandardDeviationCriterionType      value_str="NONE" />

  <!-- OT::SobolSimulationAlgorithm parameters -->
  <SobolSimulationAlgorithm-DefaultIndexQuantileLevel   value_float="0.05"           />
  <SobolSimulationAlgorithm-DefaultIndexQuantileEpsilon value_float="1.0e-1"         />
  <SobolSimulationAlgorithm-DefaultExperimentSize       value_int="1000" />

  <!-- OT::SimulationSensitivityAnalysis parameters -->
  <SimulationSensitivityAnalysis-DefaultSampleMargin value_int="400" />

  <!-- OT::SubsetSampling parameters -->
  <SubsetSampling-DefaultConditionalProbability value_float="0.1"             />
  <SubsetSampling-DefaultProposalRange          value_float="2.0"             />
  <SubsetSampling-DefaultMaximumOuterSampling   value_int="10000" />

  <!-- OT::NAIS parameters -->
  <NAIS-DefaultQuantileLevel value_float="0.25" />

  <!-- OT::CrossEntropyImportanceSampling parameters -->
  <CrossEntropyImportanceSampling-DefaultQuantileLevel value_float="0.25" />

  <!-- OT::DirectionalSampling parameters -->
  <DirectionalSampling-MeanContributionIntegrationNodesNumber value_int="255" />

  <!-- OT::AdaptiveDirectionalStratification parameters -->
  <AdaptiveDirectionalStratification-DefaultGamma                          value_float="0.5"         />
  <AdaptiveDirectionalStratification-DefaultMaximumStratificationDimension value_int="3" />
  <AdaptiveDirectionalStratification-DefaultNumberOfSteps                  value_int="2" />

  <!-- OT::LineSampling parameters -->
  <LineSampling-DefaultSearchOppositeDirection value_bool="true" />
  <LineSampling-DefaultAdaptiveImportantDirection value_bool="true" />

  <!-- OT::Analytical parameters -->
  <Analytical-LimitStateToleranceFactor value_float="1.1" />

  <!-- OT::AnalyticalResult parameters -->
  <AnalyticalResult-DefaultWidth                    value_float="1.0"           />
  <AnalyticalResult-MeanPointIntegrationNodesNumber value_int="255" />

  <!-- OT::SystemFORM parameters -->
  <SystemFORM-MaximalScaling  value_float="1.0e-5"   />
  <SystemFORM-StartingScaling value_float="1.0e-13" />

  <!-- OT::MultiFORM parameters -->
  <MultiFORM-DefaultGamma value_float="1.1" />
  <MultiFORM-DefaultDelta value_float="0.75" />
  <MultiFORM-DefaultMaximumDesignPointsNumber value_int="4" />

  <!-- OT::StrongMaximumTest parameters -->
  <StrongMaximumTest-DefaultDeltaPrecision     value_float="1.0e-7"  />
  <StrongMaximumTest-Epsilon                   value_float="1.0e-10" />

  <!-- OT::CleaningStrategy parameters -->
  <CleaningStrategy-DefaultSignificanceFactor value_float="1.0e-4"       />
  <CleaningStrategy-DefaultMaximumSize        value_int="20" />

  <!-- OT::MetaModelAlgorithm parameters -->
  <MetaModelAlgorithm-PValueThreshold         value_float="1.0e-3" />
  <MetaModelAlgorithm-ModelSelectionCriterion value_str="BIC" />
  <MetaModelAlgorithm-NonParametricModel      value_str="Histogram" />

  <!-- OT::FunctionalChaosAlgorithm parameters -->
  <FunctionalChaosAlgorithm-DefaultMaximumResidual value_float="1.0e-6"          />
  <FunctionalChaosAlgorithm-QNorm                  value_float="0.5"             />
  <FunctionalChaosAlgorithm-MaximumTotalDegree     value_int="10"    />
  <FunctionalChaosAlgorithm-BasisSize              value_int="0"     />
  <FunctionalChaosAlgorithm-Sparse                 value_bool="false" />
  <FunctionalChaosAlgorithm-FittingAlgorithm       value_str="CorrectedLeaveOneOut" />

  <!-- OT::FunctionalChaosResult parameters -->
  <FunctionalChaosResult-PrintEllipsisSize      value_int="3"    />
  <FunctionalChaosResult-PrintEllipsisThreshold value_int="1000" />
  <FunctionalChaosResult-PrintFormat            value_str="{:.7g}" />
  <FunctionalChaosResult-PrintColumnWidth       value_int="15" />

  <!-- OT::FunctionalChaosSobolIndices parameters -->
  <FunctionalChaosSobolIndices-VariancePartThreshold value_float="1.0e-2" />
  <FunctionalChaosSobolIndices-PrintFormat           value_str="{:.6f}" />
  <FunctionalChaosSobolIndices-MaximumNumberOfOutput value_int="1000" />
  <FunctionalChaosSobolIndices-PrintColumnWidth value_int="15" />

  <!-- OT::FunctionalChaosValidation parameters -->
  <FunctionalChaosValidation-ModelSelection        value_bool="false" />

  <!-- OT::LeastSquaresExpansion parameters -->
  <LeastSquaresExpansion-DecompositionMethod       value_str="QR" />

  <!-- OT::LinearModelAlgorithm parameters -->
  <LinearModelAlgorithm-DecompositionMethod        value_str="QR" />
  
  <!-- OT::LinearModelAnalysis parameters -->
  <LinearModelAnalysis-IdentifiersNumber           value_int="3" />
  <LinearModelAnalysis-PrintEllipsisThreshold value_int="20"     />
  <LinearModelAnalysis-SmallPValueFormat      value_str="{:.4e}" />
  <LinearModelAnalysis-LargePValueFormat      value_int="{:.4f}" />

  <!-- OT::LinearModelValidation parameters -->
  <LinearModelValidation-ModelSelection            value_bool="false" />

  <!-- OT::LinearModelStepwiseAlgorithm parameters -->
  <LinearModelStepwiseAlgorithm-Penalty                  value_float="2.0"      />
  <LinearModelStepwiseAlgorithm-MaximumIterationNumber   value_int="1000"        />

  <!-- OT::GeneralLinearModelAlgorithm parameters -->
  <GeneralLinearModelAlgorithm-KeepCovariance                   value_bool="true"      />
  <GeneralLinearModelAlgorithm-OptimizeParameters               value_bool="true"      />
  <GeneralLinearModelAlgorithm-UnbiasedVariance                 value_bool="true"      />
  <GeneralLinearModelAlgorithm-UseAnalyticalAmplitudeEstimate   value_bool="true"      />
  <GeneralLinearModelAlgorithm-DefaultOptimizationLowerBound    value_float="1.0e-2"  />
  <GeneralLinearModelAlgorithm-DefaultOptimizationScaleFactor   value_float="2.0"     />
  <GeneralLinearModelAlgorithm-DefaultOptimizationUpperBound    value_float="1.0e2"   />
  <GeneralLinearModelAlgorithm-MeanEpsilon                      value_float="1.0e-12" />
  <GeneralLinearModelAlgorithm-DefaultOptimizationAlgorithm     value_str="TNC"     />
  <GeneralLinearModelAlgorithm-LinearAlgebra                    value_str="LAPACK"  />

  <!-- OT::GaussianProcessFitter parameters -->
  <GaussianProcessFitter-KeepCovariance                         value_bool="true"      />
  <GaussianProcessFitter-OptimizeParameters                     value_bool="true"      />
  <GaussianProcessFitter-OptimizationNormalization              value_bool="true"      />
  <GaussianProcessFitter-UnbiasedVariance                       value_bool="true"      />
  <GaussianProcessFitter-UseAnalyticalAmplitudeEstimate         value_bool="true"      />
  <GaussianProcessFitter-DefaultOptimizationLowerBound          value_float="1.0e-2"  />
  <GaussianProcessFitter-DefaultOptimizationUpperBound          value_float="1.0e2"   />
  <GaussianProcessFitter-DefaultOptimizationNuggetLowerBound    value_float="1.0e-12"   />
  <GaussianProcessFitter-OptimizationLowerBoundScaleFactor      value_float="1.0e-3" />
  <GaussianProcessFitter-OptimizationUpperBoundScaleFactor      value_float="2.0" />
  <GaussianProcessFitter-DefaultOptimizationAlgorithm           value_str="Cobyla"     />
  <GaussianProcessFitter-LinearAlgebra                          value_str="LAPACK"  />

  <!-- OT::KrigingAlgorithm parameters -->
  <KrigingAlgorithm-LinearAlgebra   value_str="LAPACK"  />

  <!-- OT::SquaredExponential parameters -->
  <SquaredExponential-DefaultTheta value_float="1.0" />
  
  <!-- OT::AbsoluteExponential parameters -->
  <AbsoluteExponential-DefaultTheta value_float="1.0" />
  
  <!-- OT::GeneralizedExponential parameters -->
  <GeneralizedExponential-DefaultTheta value_float="1.0" />
  
  <!-- OT::MaternModel parameters -->
  <MaternModel-DefaultNu    value_float="1.5" />
  <MaternModel-DefaultTheta value_float="1.0" />
  
  <!-- OT::WeightedExperimentImplementation parameters -->
  <WeightedExperiment-DefaultSize value_int="100" />

    <!-- OT::SmolyakExperiment parameters -->
  <SmolyakExperiment-MergeRelativeEpsilon value_float="1.0e-8" />
  <SmolyakExperiment-MergeAbsoluteEpsilon value_float="1.0e-8" />
  <SmolyakExperiment-MergeQuadrature value_bool="true" />

  <!-- OT::GaussProductExperiment parameters -->
  <GaussProductExperiment-DefaultMarginalSize value_int="5" />

  <!-- OT::RatioOfUniforms parameters -->
  <RatioOfUniforms-CandidateNumber         value_int="8192"   />
  <RatioOfUniforms-MaximumMultiStart       value_int="16"     />
  <RatioOfUniforms-NormalizationSampleSize value_int="10000"  />
  <RatioOfUniforms-OptimizationAlgorithm   value_str="Cobyla" />

  <!-- OT::HyperbolicAnisotropicEnumerateFunction parameters -->
  <HyperbolicAnisotropicEnumerateFunction-DefaultQ value_float="0.4" />

  <!-- OT::MarginalTransformationEvaluation parameters -->
  <MarginalTransformationEvaluation-Simplify             value_bool="true"     />
  <MarginalTransformationEvaluation-DefaultTailThreshold value_float="0.99"    />
  <MarginalTransformationEvaluation-ParametersEpsilon    value_float="1.0e-14" />

  <!-- OT::DistributionImplementation parameters -->
  <Distribution-MinimumVolumeLevelSetBySampling   value_bool="false"              />
  <Distribution-Parallel                          value_bool="true"               />
  <Distribution-ScaleColorsDiscretePDF            value_bool="true"               />
  <Distribution-ShowSupportDiscretePDF            value_bool="true"               />
  <Distribution-UseCovarianceAdaptiveAlgorithm    value_bool="true"               />
  <Distribution-QMax                              value_float="0.85"              />
  <Distribution-QMin                              value_float="0.15"              />
  <Distribution-DefaultCDFEpsilon                 value_float="1.0e-14"           />
  <Distribution-DefaultPDFEpsilon                 value_float="1.0e-14"           />
  <Distribution-DefaultQuantileEpsilon            value_float="1.0e-12"           />
  <Distribution-DiscreteDrawPDFScaling            value_float="0.25"              />
  <Distribution-EntropySamplingMethod             value_str="MonteCarlo" />
  <Distribution-RoughnessSamplingMethod           value_str="MonteCarlo" />
  <Distribution-SupportPointStyleDiscretePDF      value_str="dot" />
  <Distribution-CharacteristicFunctionNMax        value_int="1000000" />
  <Distribution-DefaultCDFIteration               value_int="10000"   />
  <Distribution-DefaultIntegrationNodesNumber     value_int="255"     />
  <Distribution-DefaultLevelNumber                value_int="10"      />
  <Distribution-DefaultPointNumber                value_int="129"     />
  <Distribution-DefaultQuantileIteration          value_int="100"     />
  <Distribution-EntropySamplingSize               value_int="524288"  />
  <Distribution-MinimumVolumeLevelSetSamplingSize value_int="16384"   />
  <Distribution-SmallDimensionEntropy             value_int="3"       />
  <Distribution-RoughnessSamplingSize             value_int="524288"  />
  <Distribution-SmallDimensionRoughness           value_int="3"       />
  <Distribution-SmallSupport                      value_int="10"      />
  <Distribution-SupportEpsilon                    value_float="1.0e-14" />
  <Distribution-TailDependenceEpsilon             value_float="1.0e-12"/>
  <Distribution-TailDependenceMargin              value_float="1.0e-3"/>

  <!-- OT::DiscreteMarkovChain parameters -->
  <DiscreteMarkovChain-DOTArcColor value_str="black" />
  <DiscreteMarkovChain-DOTLayout value_str="dot" />
  <DiscreteMarkovChain-DOTNodeColor value_str="black" />
  <DiscreteMarkovChain-DOTNodeShape value_str="oval" />
  <DiscreteMarkovChain-ProbabilitySumPrecision value_float="1.0e-6" />

  <!-- OT::DistributionFactoryImplementation parameters -->
  <DistributionFactory-BootstrapErrorTolerance value_float="0.1"           />
  <DistributionFactory-DefaultBootstrapSize    value_int="100" />

  <!-- OT::OrderStatisticsMarginalChecker parameters -->
  <OrderStatisticsMarginalChecker-OptimizationEpsilon value_float="1.0e-7"        />
  <OrderStatisticsMarginalChecker-QuantileIteration   value_int="100" />

  <!-- OT::MaximumEntropyOrderStatisticsDistribution parameters -->
  <MaximumEntropyOrderStatisticsDistribution-CheckMarginals                  value_bool="true"           />
  <MaximumEntropyOrderStatisticsDistribution-UseApproximation                value_bool="false"          />
  <MaximumEntropyOrderStatisticsDistribution-SupportShift                    value_float="1.0e-15"       />
  <MaximumEntropyOrderStatisticsDistribution-CDFIntegrationNodesNumber       value_int="16"  />
  <MaximumEntropyOrderStatisticsDistribution-ExponentialFactorDiscretization value_int="100" />
  <MaximumEntropyOrderStatisticsDistribution-MaximumApproximationSubdivision value_int="2"   />
  <MaximumEntropyOrderStatisticsDistribution-MaximumQuantileIteration        value_int="10"  />

  <!-- OT::HMatrix parameters -->
  <HMatrix-ForceSequential      value_bool="false"            />
  <HMatrix-AdmissibilityFactor  value_float="100.0"            />
  <HMatrix-AssemblyEpsilon      value_float="1.0e-4"         />
  <HMatrix-LargestEigenValueRelativeError value_float="1.0e-1" />
  <HMatrix-RegularizationEpsilon value_float="1.0e-4"         />
  <HMatrix-RecompressionEpsilon value_float="1.0e-4"         />
  <HMatrix-ValidationError      value_float="0.0"            />
  <HMatrix-ClusteringAlgorithm  value_str="median"         />
  <HMatrix-CompressionMethod    value_str="AcaRandom"    />
  <HMatrix-FactorizationMethod    value_str="LLt"    />
  <HMatrix-FactorizationIterations value_int="10"  />
  <HMatrix-LargestEigenValueIterations value_int="10"  />
  <HMatrix-MaxLeafSize          value_int="250"  />
  <HMatrix-ValidationDump       value_int="0"    />
  <HMatrix-ValidationRerun      value_int="0"    />

  <!-- OT::GaussianProcess parameters -->
  <GaussianProcess-GibbsMaximumIteration value_int="100" />

  <!-- OT::SpectralGaussianProcess parameters -->
  <SpectralGaussianProcess-MaximalScaling    value_float="1.0e-5"          />
  <SpectralGaussianProcess-StartingScaling   value_float="1.0e-13"         />
  <SpectralGaussianProcess-CholeskyCacheSize value_int="16384" />

  <!-- OT::WhittleFactory parameters -->
  <WhittleFactory-DefaultRhoBeg             value_float="0.1"            />
  <WhittleFactory-DefaultRhoEnd             value_float="1.0e-10"        />
  <WhittleFactory-DefaultStartingPointScale value_float="1.0"            />
  <WhittleFactory-RootEpsilon               value_float="1.0e-6"         />
  <WhittleFactory-DefaultMaximumCallsNumber value_int="2000" />

  <!-- OT::BoxCoxFactory parameters -->
  <BoxCoxFactory-DefaultRhoBeg      value_float="0.1"            />
  <BoxCoxFactory-DefaultRhoEnd      value_float="1.0e-10"        />
  <BoxCoxFactory-DefaultMaximumCallsNumber value_int="2000" />
  <BoxCoxFactory-DefaultPointNumber value_int="201"  />

  <!-- OT::VisualTest parameters -->
  <VisualTest-CloudSmallSize value_int="100" />
  <VisualTest-CloudMediumSize value_int="1000" />
  <VisualTest-KendallPlot-MonteCarloSize value_int="100" />
  <VisualTest-DependenceConfidenceLevel value_float="0.95" />
  <VisualTest-DrawPairsMarginals-AxesMargin value_float="0.10" />

  <!--  OT::RandomWalkMetropolisHastings parameters -->
  <RandomWalkMetropolisHastings-DefaultAdaptationExpansionFactor   value_float="1.2" />
  <RandomWalkMetropolisHastings-DefaultAdaptationLowerBound        value_float="0.117" /> <!-- = 0.5 * 0.234 -->
  <RandomWalkMetropolisHastings-DefaultAdaptationShrinkFactor      value_float="0.8" />
  <RandomWalkMetropolisHastings-DefaultAdaptationUpperBound        value_float="0.468" /> <!-- = 2.0 * 0.234 -->
  <RandomWalkMetropolisHastings-DefaultAdaptationPeriod            value_int="30" />
  
  <!-- OT::MetropolisHastingsImplementation parameters -->
  <RandomWalkMetropolisHastings-DefaultBurnIn   value_int="1000" />

  <!-- OT::GaussianLinearCalibration parameters -->
  <GaussianLinearCalibration-Method   value_str="QR" />

  <!-- OT::LinearLeastSquaresCalibration parameters -->
  <LinearLeastSquaresCalibration-Regularization value_float="1e-12" />
  <LinearLeastSquaresCalibration-Method         value_str="QR"   />

  <!-- OT::NonLinearLeastSquaresCalibration parameters -->
  <NonLinearLeastSquaresCalibration-BootstrapSize  value_int="100" />

  <!-- OT::GaussianNonLinearCalibration parameters -->
  <GaussianNonLinearCalibration-BootstrapSize  value_int="100" />

  <!-- OT::CalibrationResults parameters -->
  <CalibrationResult-PriorLineStyle       value_str="dashed" />
  <CalibrationResult-PosteriorLineStyle   value_str="dotdash"/>
  <CalibrationResult-ObservationLineStyle value_str="solid"  />
  <CalibrationResult-PriorPointStyle       value_str="circle" />
  <CalibrationResult-PosteriorPointStyle   value_str="diamond" />
  <CalibrationResult-ObservationPointStyle value_str="fsquare" />

  <!-- OT::ARMA parameters -->
  <ARMA-MeanEpsilon value_float="1.0e-14" />

  <!-- OT::ARMALikelihoodFactoryFactory parameters -->
  <ARMALikelihoodFactory-DefaultRhoBeg             value_float="0.01"            />
  <ARMALikelihoodFactory-DefaultRhoEnd             value_float="1.0e-10"         />
  <ARMALikelihoodFactory-RootEpsilon               value_float="1.0e-6"          />
  <ARMALikelihoodFactory-DefaultMaximumCallsNumber value_int="10000" />

  <!-- OT::FittingTest parameters -->
  <FittingTest-ChiSquaredCheckSample         value_bool="true"  />
  <FittingTest-LillieforsPrecision           value_float="0.01" />
  <FittingTest-ChiSquaredMinimumBinCount     value_int="5"      />
  <FittingTest-LillieforsMinimumSamplingSize value_int="10"     />
  <FittingTest-LillieforsMaximumSamplingSize value_int="100000" />

  <!-- OT::PenalizedLeastSquaresAlgorithm parameters -->
  <PenalizedLeastSquaresAlgorithm-UseNormal  value_bool="false" />

  <!-- OT::LeastSquaresMetaModelSelection parameters -->
  <LeastSquaresMetaModelSelection-ErrorThreshold      value_float="0.0" />
  <LeastSquaresMetaModelSelection-MaximumError        value_float="0.5" />
  <LeastSquaresMetaModelSelection-MaximumErrorFactor  value_float="2.0" />
  <LeastSquaresMetaModelSelection-DecompositionMethod value_str="SVD"   />
  <LeastSquaresMetaModelSelection-SmallBasisSize value_int="10" />

  <!-- SimplicialCubature parameters -->
  <SimplicialCubature-DefaultMaximumAbsoluteError value_float="0.0" />
  <SimplicialCubature-DefaultMaximumRelativeError value_float="1.0e-5" />
  <SimplicialCubature-DefaultMaximumCallsNumber   value_int="100000" />
  <SimplicialCubature-DefaultRule value_int="3" />
  <SimplicialCubature-MarginalDiscretizationIntervalsNumber value_int="1" />

  <!-- OT::SparseMethod parameters -->
  <SparseMethod-ErrorThreshold     value_float="1.0e-3" />
  <SparseMethod-MaximumErrorFactor value_float="2.0"    />

  <!-- OT::CholeskyMethod parameters -->
  <CholeskyMethod-LargeCase value_int="128" />

  <!-- OT::Classifier parameters -->
  <Classifier-Parallel value_bool="true" />

  <!-- viewer.View parameters -->
  <View-ImageFormat value_str="png" />

  <!-- <-your-variable value_str="its value" /> -->
</openturns-configuration>
