...
\subsubsection*{Mesh dependency and regularization}
A major drawback of local softening models is the pathological mesh dependency. Mathematically, softening causes a loss of ellipticity in the differential equations, leading to strain localization in a band whose width depends on the finite element size. This results in physically unacceptable ruptures with zero energy dissipation upon mesh refinement.

To overcome this issue without resorting to complex non-local formulations, a regularization technique based on Hillerborg's specific fracture energy ($G_f$) is utilized. In the Cast3M implementation, the characteristic length of each finite element, $L_e$, is computed (e.g., using the \texttt{@LONGEF} procedure). The tensile softening parameter $B_t$ is then dynamically modified element-by-element to preserve objective energy dissipation:
\begin{equation}
    B_t^{mod} = \frac{f_t \cdot L_e}{G_f - \frac{f_t^2}{2E_0} L_e}
\end{equation}
where $f_t$ is the tensile strength and $E_0$ is the initial Young's modulus. This energetic regularization guarantees that the macroscopic THM response remains independent of the selected spatial discretization.
...
\textbf{Step 1: Solving for $\sigma_G^2$} \\
