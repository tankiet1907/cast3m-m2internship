...
# REMOVE DRYING AND AGING FOR KALIFA
...
def run_cast3m(X, run_number):
    ...
    csv_temp = os.path.join(csv_dir, f"temp_results_no_{run_number}_HR0_{hr0}_HRE1_{hre1}.csv")
    csv_pg = os.path.join(csv_dir, f"pg_results_no_{run_number}_HR0_{hr0}_HRE1_{hre1}.csv")
    csv_sw = os.path.join(csv_dir, f"sw_results_no_{run_number}_HR0_{hr0}_HRE1_{hre1}.csv")
    csv_hr = os.path.join(csv_dir, f"hr_results_no_{run_number}_HR0_{hr0}_HRE1_{hre1}.csv")
    csv_dch = os.path.join(csv_dir, f"dch_results_no_{run_number}_HR0_{hr0}_HRE1_{hre1}.csv")
    csv_hyd = os.path.join(csv_dir, f"hyd_results_no_{run_number}_HR0_{hr0}_HRE1_{hre1}.csv")
    ...
    print(f"\n[Running loop no {run_number}] Loading: HR0={hr0}, HRE1={hre1}")
...
if __name__ == "__main__":
    print("Khởi tạo danh sách các tổ hợp thông số HR0 và HRE1...")
    ...
    parameter_sets = list(itertools.product(HR0_values, HRE1_values))
    ...
    with open(combined_csv_file, mode='w', newline='', encoding='utf-8') as f:
        ...
        for i in range(N_loops):
            ...
            row_data = [hr0_val, hre1_val] + list(res)
