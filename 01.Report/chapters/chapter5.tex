...
The lognormal distribution is particularly relevant for material properties such as the Young modulus or permeability, which must remain strictly positive \cite{delarrard2010, sudret2000}.
...
A distribution is summarised by its \textbf{statistical moments}. In this work, the first two moments are prescribed as \emph{input} to the stochastic analysis, while the higher moments are \emph{computed as output} by the quadrature method (\cref{sec:quadrature}).
...
The \textbf{skewness} $\gamma_3$ and \textbf{kurtosis} $\gamma_4$ measure the asymmetry and tail heaviness of the response distribution:
...
It is a dimensionless measure that allows comparison of variability across parameters with different units and magnitudes. For concrete, typical values are $\mathrm{CoV}_E \approx 0.10$--$0.20$ for the Young's modulus \cite{delarrard2010} and $\mathrm{CoV}_K \approx 0.15$--$0.30$ for permeability.
...
where $\sigma_H^2 = C_H(\mathbf{0})$ is the field variance. It measures how similar the values at two points are as a function of their distance $\tau = \|\boldsymbol{\tau}\|$: $\rho = 1$ means identical values, $\rho \approx 0$ means nearly independent.

Two common models are used for concrete properties \cite{sudret2000, fenton2014}:
...
where $L_c > 0$ is the \textbf{correlation length}. It controls the spatial scale over which the field varies coherently:

\begin{itemize}
    \item $L_c \gg L_{\mathrm{struct}}$: the field is nearly uniform everywhere---it behaves like a random variable.
    \item $L_c \ll L_{\mathrm{struct}}$: adjacent points are nearly independent---the field fluctuates rapidly.
    \item Intermediate $L_c$: smooth but spatially varying realisations, which is the typical case for concrete \cite{delarrard2010}.
\end{itemize}
...
Monte Carlo simulation (MCS) propagates uncertainty by repeating the deterministic analysis many times with different random inputs \cite{fenton2014}. Each trial consists of three steps:
...
The TBM is natively implemented in the Cast3M \texttt{ALEA} operator, which takes as input the Gaussian parameters $(\mu_{\ln}, \sigma_{\ln})$ and the correlation length $L_c$ as input, and returns the simulated field directly as nodal values on the mesh. The lognormal transformation and assignment to \texttt{MATE} follow the procedure described in \cref{sec:lognormal-transformation}.
...
When computing exact Sobol indices is too costly, a simpler estimate can be obtained by varying each variable $X_i$ individually while holding the others at their mean. The resulting marginal variance $\hat{\sigma}_i^2$ approximates the numerator of \cref{eq:sobol-cond}, so that:
...
This approximation is valid for uncorrelated variables with negligible interaction effects. The detailed results are presented in \cref{sec:dominant-params}.
...
Each failure scenario is associated with a performance function (also called limit state function, or safety margin), denoted $G$ \cite{baroth2011}. By convention \cite{baroth2011,baroth2023}:
...
This explains why Monte Carlo simulations with spatial variability typically yield higher probabilities of failure (lower $\beta$) than the quadrature approach for the same nominal input distributions. 