...
\begin{equation}
    ...
    \label{eq:KMU_F}
\end{equation}
where $\mathbf{K}_M = \int_V \mathbf{B}^T \mathbf{D}_e\,\mathbf{B}\,dV$ is the elastic stiffness matrix.The load vector $\mathbf{F}$ receives contributions from two distinct thermo-hydric mechanisms 
(\cref{sec:TH_coupling}):
\begin{equation}
    \mathbf{F} = \mathbf{f}_{ext}
    + \underbrace{
        \int_V \mathbf{B}^T \mathbf{D}_e\,
