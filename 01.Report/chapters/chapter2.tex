...
\begin{equation}
    \tilde{\boldsymbol{\sigma}} = \mathbf{C} : \boldsymbol{\varepsilon}_e
    \label{eq:hooke}
\end{equation}
...
\end{equation}
where the thermal strain $\boldsymbol{\varepsilon}_{th}$ and the shrinkage strain $\boldsymbol{\varepsilon}_{sh}$ are defined in \cref{sec:THM_coupling}.
...
\begin{reviewblock}
\review{This is a rewrite version of the above text}

The spatial discretization of the momentum conservation 
equation (\cref{eq:final_momentum}) is carried out using 
the standard Galerkin finite element method. The 
continuous displacement field $\mathbf{u}$ is approximated 
by nodal displacements $\mathbf{U}$ using shape functions 
$\mathbf{N}$, and the strain is obtained via the 
compatibility matrix $\mathbf{B}$ such that 
$\boldsymbol{\varepsilon} = \mathbf{B}\mathbf{U}$.

Applying the principle of virtual work, the global 
system takes the form:
\begin{equation}
    [\mathbf{K}_M]\{\mathbf{U}\} = \{\mathbf{F}\}
    \label{eq:KMU_F}
\end{equation}
where $[\mathbf{K}_M] = \int_V \mathbf{B}^T \mathbf{D} 
\mathbf{B}\,dV$ is the global stiffness matrix and 
$\{\mathbf{F}\}$ is the equivalent nodal force vector. 
In the THM context, this force vector includes 
contributions from the thermo-hydric fields:
\begin{equation}
    \{\mathbf{F}\} = \{\mathbf{F}\}^{ext} 
    + \{\mathbf{F}\}^{th}(T) 
    + \{\mathbf{F}\}^{sh}(p_g, p_c) 
    + \{\mathbf{F}\}^{p}(p_g, p_c)
    \label{eq:F_THM}
\end{equation}
where $\{\mathbf{F}\}^{ext}$ groups the prescribed 
surface forces and gravity, $\{\mathbf{F}\}^{th}$ is the 
thermal dilation pseudo-force derived from 
\cref{eq:eps_th}, $\{\mathbf{F}\}^{sh}$ is the 
shrinkage pseudo-force from \cref{eq:eps_sh}, and 
$\{\mathbf{F}\}^{p}$ is the pore pressure loading from 
\cref{eq:eff_stress}. These three last terms are 
computed from the TH solution at each time step.

In Cast3M, this linear system is assembled and solved 
using the \texttt{RIGI} and \texttt{RESO} operators.
\end{reviewblock}
...
\begin{equation}
    \boldsymbol{\sigma} = (1 - \mathcal{D})\, \tilde{\boldsymbol{\sigma}}
    \label{eq:damage_stress}
\end{equation}

The term \textit{effective stress} here refers to the Mazars concept (stress in the undamaged skeleton), and should not be confused with the Biot effective stress $\boldsymbol{\sigma}''$ used in the THM framework (\cref{sec:coupling}), which accounts for pore pressure effects.
...
Due to the damage evolution, the stress-strain relationship becomes non-linear and. The mechanical equilibrium \cref{eq:KMU_F} \draft{cannot be solved in a single step like the elastic model. By introducing the non-linear complementary stress $\boldsymbol{\sigma}_{nl}$, the equilibrium} is rewritten as:
\begin{equation}
...
\begin{reviewblock}
\review{This is a rewritten version of above text}
Due to damage evolution, the constitutive relation 
becomes non-linear and \cref{eq:KMU_F} cannot be solved 
in a single step. The equilibrium is enforced iteratively 
using a Newton-Raphson-type algorithm. At each iteration 
$i$, the imbalance vector is evaluated:
\begin{equation}
    \{\mathbf{R}\}^i = \{\mathbf{F}\} 
    - \int_V \mathbf{B}^T \boldsymbol{\sigma}^i\,dV
\end{equation}
where $\{\mathbf{F}\}$ includes all thermo-hydric 
contributions as defined in \cref{eq:F_THM}. The 
correction is obtained from:
\begin{equation}
    [\mathbf{K}_M]\{\delta\mathbf{U}\}^{i+1} 
    = \{\mathbf{R}\}^i
\end{equation}
The displacements and damage variable $\mathcal{D}$ are 
updated until convergence 
($\|\mathbf{R}\| < \epsilon\,F_{ref}$). In Cast3M, 
this iterative procedure is handled by the 
\texttt{PASAPAS} operator.
\end{reviewblock}
...
Beyond the free strains ($\boldsymbol{\varepsilon}_{th}$, $\boldsymbol{\varepsilon}_{sh}$), the pore pressure directly modifies the stress state in the solid skeleton through the concept of effective stress, which is defined as:
\begin{equation}
    \boldsymbol{\sigma}'' = \boldsymbol{\sigma} + \mathbf{I}\,\alpha\,p_s
    \label{eq:eff_stress}
\end{equation}
where $\boldsymbol{\sigma}$ is the total stress tensor and $p_s$ is the average pore pressure defined in \cref{eq:ps_shrinkage}.
...
\end{equation}
where $\mathbf{g}$ is the gravitational acceleration and 
...
The total stress $\boldsymbol{\sigma}^{Total}$ is related to the effective stress via Bishop's relation (Eq.~\eqref{eq:eff_stress_final}), which in turn depends on the primary variables $p_g$ and $p_c$.
...
\subsubsection{Combined mechanism and HPC vulnerability} \label{sec:combined_mech}
...
\begin{table}[htbp]
\centering
\caption{Factors contributing to HPC vulnerability to spalling}
\label{tab:HPC_vulnerability}
\begin{tabular}{p{3.5cm}p{4cm}p{5cm}}
\textbf{HPC property} & \textbf{Model parameter} & 
\textbf{Effect on spalling} \\
Low water-to-cement ratio & Low intrinsic permeability $K_0$ & Hinders vapor evacuation $\rightarrow$ higher $p_g$ \\[4pt]
Dense pore structure & Low porosity $\phi_0$ & Less storage volume for vapor $\rightarrow$ rapid pressure build-up \\[4pt]
High compressive strength & Higher $E_0$ & More elastic energy stored $\rightarrow$ more violent release \\[4pt]
Higher brittleness & Lower $G_f / f_t$ ratio & Less energy dissipation before fracture $\rightarrow$ sudden failure \\[4pt]
Higher initial saturation & Higher $S_{l,0}$ & More water available for evaporation $\rightarrow$ higher $p_g$ \\
\end{tabular}
\end{table}

\subsubsection{Role of damage-permeability coupling}

\review{This section is currently under development and will be refined as the study progresses.}

The competition between pore pressure build-up and damage-induced permeability increase (Eq.~\eqref{eq:perm_damage}) plays a decisive role in determining whether spalling occurs:

\begin{itemize}
    \item \textbf{Scenario 1 --- No spalling:} If microcracking develops early (e.g., from thermal stresses), the damage variable $D$ increases, causing an increase in permeability via the factor $10^{A_d \cdot d}$. This creates additional escape paths for vapor, \textit{reducing} pore pressure before it can reach the critical level. This self-regulating mechanism is more effective in normal-strength concrete, which has higher initial permeability and cracks more readily.
    
    \item \textbf{Scenario 2 --- Spalling:} In HPC, the low initial permeability causes rapid pressure build-up, while the high tensile strength delays cracking. By the time the first cracks form, the pore pressure may already exceed the tensile strength, leading to sudden, explosive failure before permeability can increase sufficiently.
\end{itemize}

This explains the paradox that \textit{stronger} concrete is often \textit{more} vulnerable to spalling than weaker concrete.

\subsubsection{Factors influencing spalling}
...
\begin{itemize}
    \item \textbf{Moisture content:} Higher initial moisture provides more water for evaporation and therefore higher peak pore pressures;
    \item \textbf{Heating rate:} Faster heating creates steeper thermal gradients and allows less time for pressure relief through transport;
    \item \textbf{External loading:} Compressive loading superimposes additional stresses to the thermally induced ones, reducing the margin before tensile failure;
    \item \textbf{Concrete strength and permeability:} As discussed above, higher strength and lower permeability increase vulnerability;
    \item \textbf{Aggregate type:} Mineralogical character of aggregates affects thermal expansion compatibility and crack initiation;
    \item \textbf{Geometry and element size:} Thinner sections and edges are more prone to spalling.
\end{itemize}
