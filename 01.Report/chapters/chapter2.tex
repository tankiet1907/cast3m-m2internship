...
\begin{equation}
    ...
    \label{eq:spalling}
\end{equation}
where $\sigma_{thermal}$ is the thermal stress from differential dilation, $\sigma_{pore}$ the stress from pore-pressure loading, and $f_t(T)$ the temperature-degraded tensile strength. A comprehensive breakdown of this combined spalling mechanism is provided in \cref{sec:THM_spalling_app} of \cref{appendixA}.


\section{Improved thermo-hydro-chemical (THC) model} \label{sec:thcmodel}

\subsection{Motivation}
In standard Thermo-Hydric (TH) models, dehydration typically enters only as a prescribed temperature-dependent source term $\dot m_{dehyd}(T)$ and the porosity follows a simple linear law (as detailed in \cref{appendixA}). This treatment ignores three facts that strongly influence spalling: (i) the \emph{chemical state} of the cement paste before the accident depends on its hydration/curing history; (ii) the release of chemically bound water from the C--S--H is the chemical process that controls porosity, permeability and strength at high temperature; and (iii) chemical damage is essentially \emph{irreversible}. The improved Thermo-Hydro-Chemical (THC) model adopted here, following Scium\`e, Moreira and Dal~Pont \cite{sciume2024}, resolves the chemical field explicitly and couples it strictly to the thermal and hydric fields.
