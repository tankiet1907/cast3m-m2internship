...
Beyond the free strains ($\boldsymbol{\varepsilon}_{th}$, $\boldsymbol{\varepsilon}_{sh}$), the pore pressure directly modifies the stress state in the solid skeleton through the concept of effective stress. Following Bishop's relation:
\begin{equation}
    \boldsymbol{\sigma}' = \boldsymbol{\sigma}^{Total} + \mathbf{I}\,\alpha\,p_s
    \label{eq:eff_stress}
\end{equation}
where $\boldsymbol{\sigma}^{Total}$ is the total stress tensor and $p_s$ is the average pore pressure (\cref{eq:ps_shrinkage}). This relation decomposes the total stress into the part carried by the solid skeleton ($\boldsymbol{\sigma}'$) and the part carried by the pore fluids ($\alpha\,p_s$).

The effective stress $\boldsymbol{\sigma}'$ is the stress that governs the mechanical behaviour of the skeleton: it is the input to the Mazars damage model (\cref{eq:damage_stress}). In heated concrete, gas pressure builds up due to evaporation and dehydration (\cref{sec:pore_pressure}). This increase in $p_s$ enters the load vector as an additional tensile loading on the skeleton (\cref{eq:F_full}), shifting $\boldsymbol{\sigma}'$ toward tension and promoting damage initiation when $f_t$ is exceeded.
