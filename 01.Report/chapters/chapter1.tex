...
\textsc{Cast3M} (or Castem) is an open-source finite-element code developed at CEA, which is used in this work to implement the coupled THM constitutive laws and the custom multiphysics formulation.
...
\textsc{OpenTURNS} is the open-source library used to manage the uncertainty in the material parameters: it generates the random samples, propagates them through the model and computes the sensitivity (Sobol) indices and the reliability measures

\subsection{Supporting tools}
\textsc{Python} (in Visual Studio Code) is used to automate the calibration workflow and to post-process the \textsc{Cast3M} results, and \LaTeX{} is used to typeset this report.
...
Chapter 6 summarises the main findings and outlines perspectives for future work. Detailed derivations are collected in
