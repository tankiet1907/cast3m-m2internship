...
The evolution of the hydration degree $\Gamma$ is governed by an Arrhenius-type equation:
\begin{equation}
    \frac{\mathrm{d}\Gamma}{\mathrm{d}t} = \mathcal{A}(\Gamma) \cdot \beta_{\mathrm{RH}} \cdot \exp\left( -\frac{E_a}{R T} \right)
\end{equation}
where $E_a$ is the activation energy, and $\beta_{\mathrm{RH}}$ accounts for the moisture effect. The macroscopic chemical affinity $\mathcal{A}(\Gamma)$ drives the reaction speed depending on the current hydration state \cite{sciume2024}:
\begin{equation}
...
\end{equation}
where $\langle \cdot \rangle_+$ is the positive part operator. The parameters $A_i, A_p$ regulate the initial and peak affinity, $\Gamma_p$ is the hydration degree at the peak, and $\zeta$ controls the deceleration phase.
...
\end{equation}
where $K_0$ is the initial intrinsic permeability, $A_\Gamma$ is a coefficient defining the maximum increase in permeability upon total dehydration, and $B_K$ modifies the shape of the permeability gain. This formulation elegantly replaces direct temperature- or damage-dependent multipliers by relying strictly on the microstructural state.
...
\end{equation}
The relaxation parameter $\theta \in [1]$ dictates the stability and accuracy of the scheme:
\begin{itemize}
