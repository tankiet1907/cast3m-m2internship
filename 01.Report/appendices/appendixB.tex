...
To reduce the computational cost and accelerate statistical convergence, \textbf{Latin Hypercube Sampling (LHS)} is utilized as a variance-reduced alternative \cite{mckay1979,baudin2017}. In LHS, the cumulative distribution function (CDF) of each random variable is divided into $N$ equiprobable strata. Exactly one sample is drawn from each stratum, guaranteeing that the input space is covered evenly across the entire range of the probability distributions. This ensures the output statistics stabilize with markedly fewer evaluations compared to standard Monte Carlo.
...
Each method independently yields a probability of failure, obtained from the reconstructed CDF $\tilde{F}_G$ as
